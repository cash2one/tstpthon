...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Demand.company_province'
        db.delete_column(u'demand_demand', 'company_province_id')

        # Deleting field 'Demand.company_country'
        db.delete_column(u'demand_demand', 'company_country_id')

        # Adding M2M table for field company_countries on 'Demand'
        m2m_table_name = db.shorten_name(u'demand_demand_company_countries')
        ...
        db.create_unique(m2m_table_name, ['demand_id', 'country_id'])

        # Adding M2M table for field company_provinces on 'Demand'
        m2m_table_name = db.shorten_name(u'demand_demand_company_provinces')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('demand', models.ForeignKey(orm[u'demand.demand'], null=False)),
            ('province', models.ForeignKey(orm[u'area.province'], null=False))
        ))
        db.create_unique(m2m_table_name, ['demand_id', 'province_id'])


    def backwards(self, orm):
        # Adding field 'Demand.company_province'
        db.add_column(u'demand_demand', 'company_province',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['area.Province'], null=True, blank=True),
                      keep_default=False)

        # Adding field 'Demand.company_country'
        db.add_column(u'demand_demand', 'company_country',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['area.Country'], null=True, blank=True),
                      keep_default=False)

        # Removing M2M table for field company_countries on 'Demand'
        db.delete_table(db.shorten_name(u'demand_demand_company_countries'))

        # Removing M2M table for field company_provinces on 'Demand'
        db.delete_table(db.shorten_name(u'demand_demand_company_provinces'))
