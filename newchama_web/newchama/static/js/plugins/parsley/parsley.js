    ...
    */
    this.messages = {
      ...
      , type: {
          ...
          , float:      "THis value should be a valid number."
          , percent:    "This value should be enter 0-100."
      ...
      , type: function ( val, type ) {
        ...
        switch ( type ) {
          case 'number':
            ...
            break;
          case 'float':
            regExp = /^(?:\d{1,3}(\.\d{1,2})?|1000)$/;
            break;
          case 'percent':
            regExp = /^(?:\d{1,2}(\.\d{1,2})?|100|100.0|100.00)$/;
            break;
          case 'digits':
            regExp = /^\d+$/;
            break;
...
//      for (var i = 0; i < $( this.ulError ).length; i ++) $( this.ulError ).remove();            //Terry for double error occuar add 20140922
      $( this.ulError ).remove();
    }
    ...
    */
    , manageError: function ( constraint ) {
      ...
      if ( !$( this.ulError + ' .' + liClass ).length ) {
          //Terry add custom error message
          var customErrorMessage = message;
          try {
              if (typeof $(this.element[0]).attr("data-error-message") != "undefined")
                customErrorMessage = $(this.element[0]).attr("data-error-message");//this.element[0].attributes.data-error-message.value;
          } catch (e) {}
          //Terry add end
        liError[ liClass ] = customErrorMessage;
    ...
    */
    , manageErrorContainer: function () {
      ...
      if ( 'undefined' !== typeof errorContainer ) {
          //if radio or checkbox , add the tip of the parent tag
          !this.isRadioOrCheckbox ? $( errorContainer[0] ).append( ulTemplate ) : $( errorContainer[0] ).parent().append( ulTemplate );
//        $( errorContainer[0] ).append( ulTemplate );            //Terry handle much error occuar add 20141028
