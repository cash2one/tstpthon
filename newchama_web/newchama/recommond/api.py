...
from services.models import Project,ProjectKeyword,ProjectKeywordAdmin
from services.models import CompanyInvestmentField,Company,CompanyInvestmentHistory
from services.views import convert_finance_type_to_num,convert_growth_type_to_num
from services.models import Demand, DemandIndustry
import datetime
    ...
    for p_kv_item in project_kv_list+project_admin_kv_list:
        if company_invest_field.tags:
            if p_kv_item not in notkey:
                if p_kv_item in company_invest_field.tags:
                    _is_ok = True
    return _is_ok
...
def GetRecommondScoreItem(project,company):
    ...
    if _recommond_score.company_score_industry_keyword >0:
        _recommond_score.target_reason = 'Company Investment Criteria' #有相关投资需求 改成  持续关注此投资领域　２０１５０６０９

    if _recommond_score.sum_project_score >=PROJECT_SCORE_LEVEL:
        _recommond_score.target_reason = 'Active Buy-Side Mandates' #有活跃的买方需求   改成　正在寻找相关标的　２０１５０６０９
