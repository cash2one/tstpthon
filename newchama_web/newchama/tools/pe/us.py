...
def save_single_stock_data(quote,conn,cur):
    ...
    if r and r[2]:
        ...
        _afterHoursTime=parse(quote['afterHoursTime']).strftime('%Y-%m-%d %H:%M:%S')
...
values = {'page':page_idx,'size':page_size,'order':'desc','orderby':'percent','type':'0,1,2,3','_':int(time.time())}
data = urllib.urlencode(values)
