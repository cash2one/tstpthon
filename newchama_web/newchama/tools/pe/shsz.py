...
def save_single_stock_data(quote,conn,cur):
    ...
    if quote['exchange']=='SH':
        quote['exchange']='SHSE'
        quote['symbol']=quote['symbol'].replace('SH','')
        quote['code']=quote['code'].replace('SH','')
    elif quote['exchange']=='SZ':
        quote['exchange']='SZSE'
        quote['symbol']=quote['symbol'].replace('SZ','')
        quote['code']=quote['code'].replace('SZ','')
...
def get_single_stock_data(item,conn,cur):
    ...
    try:
        ...
        #print u'#%-6s%-20s PE:%-15s Market Capital:%s'%(str(current_idx),quote['exchange']+':'+quote['symbol'],quote['pe_ttm'],quote['marketCapital'])
...
def working():
    ...
    #print 'Open Datebase Connection'
...
values = {'page':page_idx,'size':page_size,'order':'desc','orderby':'percent','type':'11,12','_':int(time.time())}
data = urllib.urlencode(values)
