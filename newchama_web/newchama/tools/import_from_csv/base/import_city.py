...
for row in csv.reader(open(fname,'rU'), delimiter=';'):
	if row[6] !="":
		...
		cur.execute("select count(*) from area_city where name_en='"+row[6].strip().replace("'","''")+"'")
		r = cur.fetchone()
		if r[0]==0:

			cur.execute("select id from area_province where name_en='"+row[4].strip().replace("'","''")+"'")
			t = cur.fetchone()
			fid=t[0]

			sql_str="insert into area_city values(null,%d,'%s','%s')" % (fid,row[6].strip().replace("'","''"),'')
