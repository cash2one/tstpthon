...
def mapper_item(soup):
    ...
    _invest_company_list=soup.select('#company-member-list tr')[2].select('td a')
    ...
    item.field=soup.select('#company-member-list tr')[4].select('td')[1].get_text()
    item.ratio=soup.select('#company-member-list tr')[5].select('td')[1].get_text()    
...
def save_item(session,query,item):
    c = query.filter_by(company=item.company,happen_time=item.happen_time,money=item.money).first()
...
session,query=bind_db('itjuzi_merger')
...
for num in range(start_id,end_id+1):
    print num
    try:
        _soup=get_html('http://itjuzi.com/merger/'+str(num))
